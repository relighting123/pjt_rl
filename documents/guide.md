...
3.  **Usage with `rts` command**:
    Once installed, you can use the `rts` command directly.
...
```bash
# 기본 학습 (데이터셋 전체 활용)
rts train --data_dir ./data --config config.yaml

# 특정 시나리오만 집중 학습
rts train --scenario scn#1 --data_dir ./data
...
```bash
# 특정 시나리오 추론
rts infer --mode rl --scenario scn#1 --model_path models/ppo_eqp_allocator

# 다른 모델 파일 지정
rts infer --mode rl --model_path models/my_final_model --scenario scn#2
```

**Heuristic Mode (규칙 기반 전문가):**
```bash
# 기본 휴리스틱 알고리즘 실행
rts infer --mode heuristic --scenario scn#1

# 결과 파일명 지정
rts infer --mode heuristic --scenario scn#2 --output results/scn2_heuristic.csv
```
...
rts infer --mode rl --timekey 20260211000001

# 휴리스틱 모드로 DB 데이터 추론
rts infer --mode heuristic --timekey 20260211000001
```
'''
### DB 데이터 동기화 (Sync-DB)
...
rts sync-db --timekey 20260211000001 --data_dir ./data
```
